...
int removeCourse(int clientSocket, char *auth) {
    struct course my_course, temp;
    int openFD = open("course.txt", O_RDWR, 0644);
    ...
    int foundIndex = -1; // To keep track of the position of the found course

    while (read(openFD, &temp, sizeof(temp)) == sizeof(temp)) {
        if ((strcmp(my_course.course_id, temp.course_id) == 0) && (strcmp(my_course.prof_id, temp.prof_id) == 0)) {
            found = true;
            foundIndex = lseek(openFD, 0, SEEK_CUR) - sizeof(temp); // Get the position of the found course
            break;
        }
    }
    ...
    lseek(openFD, foundIndex, SEEK_SET);
